...
export const questions: Question[] = [
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation:
      "The technique is called prefilling. In the Messages API you append an `assistant` message as the LAST entry in the `messages` array, and Claude continues from exactly where that text leaves off. With `[{user: 'How do I reverse a list in Python?'}, {assistant: 'To reverse a list'}]`, the response starts ' in Python, use list.reverse() or list[::-1]...' instead of 'Certainly! To reverse...'. The greeting is structurally impossible because the model has already 'said' something non-greeting and just keeps going.\n\nMechanics that trip people up: (1) the assistant prefill must be the last message in the array; (2) the response you get back contains only the continuation, not your prefill, so concatenate `prefill + response.content[0].text` if showing the whole answer; (3) trailing whitespace matters because Claude resumes from the exact character (`'To reverse a list'` vs `'To reverse a list '` produce different continuations); (4) common patterns are prefilling `{` to force JSON, `<analysis>` to force a tag structure, or the first content word of the expected answer to skip pleasantries.\n\nWhy the other options lose: system-prompt bans are whack-a-mole (the model swaps 'Certainly!' for 'Of course!', 'Sure thing!', 'Happy to help!'); lower temperature makes the greeting MORE consistent, not less, because greetings are the high-probability opening at any temperature; post-process strip is brittle (you must enumerate every variant) and you have already paid the tokens and latency.\n\nRecognition pattern for the exam: whenever a question is about shaping the start or structure of the output deterministically, prefilling is almost always the answer. System prompts shape tendencies; prefilling shapes the actual tokens that come out next.",
  },
